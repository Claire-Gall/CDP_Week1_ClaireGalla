...
const kleinBottle = new THREE.Mesh(
  ...
  new THREE.MeshBasicMaterial({ color: colors[2], wireframe: true }) // wireframe shows structure nicely
);
kleinBottle.position.set(6, 4, 6);
scene.add(kleinBottle);
    // Add ambient and directional light for dramatic effect
    const ambientLight = new THREE.AmbientLight(0xffffff, 0.5);
    scene.add(ambientLight);
    const directionalLight = new THREE.DirectionalLight(0xffffff, 1);
    directionalLight.position.set(5, 10, 7);
    scene.add(directionalLight);
    // Create a wireframe grid for reference
    const grid = new THREE.GridHelper(20, 40, 0x888888, 0x888888);
    scene.add(grid);
